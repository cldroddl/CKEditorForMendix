...
│   ├── loadCKEditor.ts        ckeditor.js를 1회 로드(기본: 번들된 사본), 전역 window.CKEDITOR 관리
...
rollup.config.mjs             pwt 기본 설정에 병합 — 빌드 시 CKEditor 트리를 assets/ckeditor/로 복사
```

-   **CKEditor 4 엔진은 저장소에 없지만, 빌드가 `.mpk` 안에 번들합니다.** `rollup.config.mjs`가 dev 의존성
    `ckeditor4@4.22.0` + `ckeditor-wordcount-plugin`에서 런타임 트리를 `assets/ckeditor/`로 복사 → 위젯이 자기
    에셋(`<app>/widgets/ckeditorformendix/richtext/assets/ckeditor/ckeditor.js`)에서 로드. 외부 요청 0, 오프라인 OK.
    `editorScriptUrl` 속성에 값을 넣으면 다른 4.22.0 빌드(CDN 등)로 대체.
...
packages/<widget>/dist/<version>/
      ...
      └── (mpk 내부)
          ...
          ├── assets/ckeditor/           ← 번들된 CKEditor 4.22.0 런타임 (rollup.config.mjs가 복사)
          └── package.xml
```

-   **위젯 하나당 `.mpk` 하나.** 레거시는 한 `.mpk`에 에디터+뷰어를 같이 담았지만, pwt는 npm 패키지 = `.mpk` 1:1이라 파일이 두 개입니다. 앱에는 둘 다 임포트합니다.
-   에디터 `.mpk`는 번들된 CKEditor 때문에 약 2.7MB (뷰어는 ~26KB). CKEditor는 git에 없고 빌드 시 dev 의존성에서 복사됩니다.
